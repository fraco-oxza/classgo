...
def add(name, nick, teacher, link):
    ...
    c.execute(
        f"INSERT INTO class VALUES (\"{name}\", \"{teacher}\", \"{link}\")")
    ...
    while True:
        try:
            ...
            click.echo("\n" + click.style("Error", bg="red", fg="white") +
                       click.style(": Por favor siga las instrucciones", bold=True))

        except AssertionError:
            click.echo("\n" + click.style("Error", bg="red", fg="white") +
                       click.style(": los números deben estar en el rango 1-7", bold=True))
    ...
    for d in dates:
        while True:
            ...
            click.pause("\nToque cualquier tecla para volver a intentarlo.")
        c.execute(
            f"INSERT INTO bouquet VALUES (\"{d}\", \"{list_time[0]}\", \"{list_time[1]}\", \"{name}\")")
    db.commit()
    db.close()
